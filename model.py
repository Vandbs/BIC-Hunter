...
class GCN(nn.Module):
    def __init__(self, device,in_channels, hidden_channels, out_channels, pyg):
        ...
        self.device = device
        self.bert_model = AutoModel.from_pretrained("microsoft/codebert-base")  # 使用预训练的 CodeBERT 模型。
...
#第一段代码的作用是根据给定的节点的 token ID，利用 BERT 模型生成节点的嵌入向量，并将这些嵌入向量赋值给图数据对象中对应节点的特征（x）。
        ...
        edge_weight = 4.0 * torch.ones(edge_index.shape[1], dtype=torch.float32, device=self.device)
        x = self.conv1(homogeneous_data.x, edge_index, edge_weight)
