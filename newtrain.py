...
def init_model(device, pyg):
    ...
    gcnModel = GCN(device, 768, 1, 768 * 2, pyg)  # 初始化了一个 HAN 模型。
    gcnModel = gcnModel.to(device)  # 将模型移动到指定的设备上
    ...
    optimizer = torch.optim.Adam(
        chain(gcnModel.parameters(), rankNetModel.parameters()), lr=5e-6
    )  # 定义了一个优化器，使用的是 Adam 算法。优化器会更新 hanModel 和 rankNetModel 的参数以最小化损失函数。学习率设置为 5e-6。

    return gcnModel, rankNetModel, optimizer, criterion

def init_model2(device, pyg):
    criterion = torch.nn.BCELoss()  # 二元交叉熵损失函数，一种常用于二分类问题的损失函数。
    gcnModel2 = GCN2(device, 768, 1, 768 * 2, pyg)  # 初始化了一个 HAN 模型。
    gcnModel2 = gcnModel2.to(device)  # 将模型移动到指定的设备上
    rankNetModel = rankNet(768 * 2)  # 初始化了一个 rankNet 模型，输入维度是 768 * 2。
    rankNetModel = rankNetModel.to(device)  # 将 rankNetModel 移动到指定的设备上

    optimizer = torch.optim.Adam(
        chain(gcnModel2.parameters(), rankNetModel.parameters()), lr=5e-6
    )  # 定义了一个优化器，使用的是 Adam 算法。优化器会更新 hanModel 和 rankNetModel 的参数以最小化损失函数。学习率设置为 5e-6。

    return gcnModel2, rankNetModel, optimizer, criterion
...
def get_all_batchlist(mini_graphs, k, max_pair):  # 这个函数的主要目的是将 mini_graphs 中的所有子图分割成 k 个部分，然后为每个部分生成一组批次列表。
    ...
    for sub_minigraph in all_sub_minigraphs:
        all_pairs = get_all_pairs1(sub_minigraph, max_pair)  # 使用 get_all_pairs 函数为 sub_minigraph 生成一组对，最多 max_pair 对。
...
def get_all_batchlist2(mini_graphs, k, max_pair, mask):  #制作成批量的生成图对
    all_batch_list = []
    pair_cnt = 0
    all_sub_minigraphs, all_sub_fdirs = divide_minigraphs(mini_graphs, k)  #分成一份，格式双重列表[[test1，test2...]]

    # print(all_sub_fdirs)

    for sub_minigraph in all_sub_minigraphs:
        all_pairs = get_all_pairs(sub_minigraph, mask, max_pair)  #制作每生成图对，并赋予ground truth probability
...
# %%
# %%
def train_batchlist(batches, gcnModel, rankNetModel, optimizer, criterion, device, xxx, gcnModel2):
    # 这个函数的主要目的是训练模型并计算损失。   在这里找出来所有输入模型的pyg和权重最佳
    ...
    for batch in batches:
        if(zzz<=xxx):
            ...
            x = gcnModel(pyg1, del_index1)  # 通过 hanModel 计算 pyg1 和 pyg2 的输出。
            y = gcnModel(pyg2, del_index2)
        ...
        if (zzz > xxx):
            ...
            probs = batch.probs.to(device)
            x = gcnModel2(pyg1, del_index1)  # 通过 hanModel 计算 pyg1 和 pyg2 的输出。
            y = gcnModel2(pyg2, del_index2)
...
def validate_batchlist(batches, gcnModel, rankNetModel, criterion, device):
    all_loss = []
    gcnModel.eval()
...
def do_cross_fold_valid(device, K):
    ...
    for i in range(0, len(all_data_list)):
        ...
        train_x, train_y, psx, train_all_nodes, train_all_fdirs, train_all_labels = get_vectorizedata(trainset)

        mask = get_noise(train_x, train_y, psx, train_all_nodes, train_all_fdirs, train_all_labels)
        ...
        all_batch_list2, all_sub_fdirs, pair_cnt = get_all_batchlist2(  # 制作成批量的生成图对
            mini_graphs, 1, max_pair, mask
        )

        all_batch_list1, all_sub_fdirs, pair_cnt = get_all_batchlist(
            mini_graphs, 1, max_pair=max_pair
        )
        if all_batch_list1 and isinstance(all_batch_list1[0], list):
            # 使用 len() 函数获取 all_batch_list1[0] 中元素的个数
            xxx = len(all_batch_list1[0])
        #all_batch_list = all_batch_list1 + all_batch_list2
        if all_batch_list1 and all_batch_list2:
            # 将 all_batch_list2 的第一个元素合并到 all_batch_list1 的第一个元素后面
            all_batch_list1[0].extend(all_batch_list2[0])
        ...
        gcnModel, rankNetModel, optimizer, criterion = init_model(
            device, all_batch_list1[0][0].pyg1
        )
        gcnModel2, rankNetModel, optimizer, criterion = init_model2(
            device, all_batch_list1[0][0].pyg1
        )
        ...
        for epoch in range(epochs):
            ...
            total_fp3 = 0
            total_t = 0
            # print("位置1")图在下面这里进入模型
            # print(device)
            total_train_loss = total_train_loss + train_batchlist(
                all_batch_list1[0], gcnModel, rankNetModel, optimizer, criterion, device, xxx, gcnModel2
            )

            # print(all_batch_list[0])
            # print("位置2")
            # print(device)
...
def do_cross_project_predict(device):
    ...
    if all_batch_list and isinstance(all_batch_list[0], list):
        # 使用 len() 函数获取 all_batch_list1[0] 中元素的个数
        xxx = len(all_batch_list[0])
    if all_batch_list and all_batch_list2:
        # 将 all_batch_list2 的第一个元素合并到 all_batch_list1 的第一个元素后面
        all_batch_list[0].extend(all_batch_list2[0])
    ...
    gcnModel2, rankNetModel, optimizer, criterion = init_model(
        device, all_batch_list[0][0].pyg1
    )
    epochs = 20
    all_info = []
    for epoch in range(epochs):
        ...
        total_t = 0
        total_train_loss = total_train_loss + train_batchlist(
            all_batch_list[0], gcnModel, rankNetModel, optimizer, criterion, device, xxx, gcnModel2
        )
        eval(trainset, dir_to_minigraphs, gcnModel, rankNetModel, device)
        tp1, fp1, t = eval_top(
            trainset,
