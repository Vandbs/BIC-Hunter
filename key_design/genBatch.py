...
def combineGraph(pygs, doc2vec):
    ...
    if doc2vec == None:
        ...
        data["add_node"].x = torch.zeros((0, 768), dtype=torch.float)
        data["del_node"].x = torch.zeros((0, 768), dtype=torch.float)
    ...
    for pyg in pygs:
        ...
        if doc2vec == None:
            ...
            data["add_node"].x = torch.cat((data["add_node"].x, pyg["add_node"].x), 0)
            data["del_node"].x = torch.cat((data["del_node"].x, pyg["del_node"].x), 0)
        ...
        if doc2vec == None:
            ...
            delCnt = delCnt + pyg["del_node"].x.size()[0]
            addCnt = addCnt + pyg["add_node"].x.size()[0]

    return data, torch.tensor(del_indexes)


def combinePair(allPairs, max_nodes=256, doc2vec=None):
    batches = []
    pygs1 = []
    pygs2 = []
    ...
    for pair in allPairs:
        ...
        if doc2vec == None:
            ...
            curCnt = (
                pyg1["add_node"].x.size()[0]
                + pyg1["del_node"].x.size()[0]
                + pyg2["add_node"].x.size()[0]
                + pyg2["del_node"].x.size()[0]
            )
        if nodeCnt + curCnt > max_nodes:
            batchPyg1, delIndex1 = combineGraph(pygs1, doc2vec)
            batchPyg2, delIndex2 = combineGraph(pygs2, doc2vec)
