...
from genPyG import get_graph_data, get_graph_data1
...
# get all train pairs
def get_all_pairs(all_data_map, max_cnt=1000, doc2vec=None):
    all_pairs = []
        ...
        for i in range(len(graphs1)):
            for j in range(i + 1, len(graphs1)):
                if doc2vec == None:
                    ...
                    pyg1 = get_graph_data1(graphs1[i], doc2vec)
                    pyg2 = get_graph_data1(graphs1[j], doc2vec)
...
def get_dir_to_minigraphs(all_data_map, doc2vec=None):
    dir_to_minigraphs = {}
    for key, graphs in all_data_map.items():
        ...
        for i in range(len(graphs)):
            if doc2vec == None:
                ...
                pyg = get_graph_data1(graphs[i], doc2vec)
