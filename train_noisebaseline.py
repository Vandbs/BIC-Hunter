...
seed = 2023
import os
med = 'cl' # also try 'oss','gbs','e2sc','clni','if','rsds'
os.environ["PYTHONHASHSEED"] = str(seed)
os.environ["CUBLAS_WORKSPACE_CONFIG"] = ":4096:8"
sys.path.append('/noise_baseline')
import random
...
from noise_baseline.noise_baseline import *

import nltk
nltk.data.path.append('/home/user/nltk_data/') #change to your path
...
# %%
def get_all_batchlist(mini_graphs, k, max_pair, mask):  # 这个函数的主要目的是将 mini_graphs 中的所有子图分割成 k 个部分，然后为每个部分生成一组批次列表。
    all_batch_list = []
    pair_cnt = 0
    all_sub_minigraphs, all_sub_fdirs = divide_minigraphs(mini_graphs, k)

    for sub_minigraph in all_sub_minigraphs:
        all_pairs = get_all_pairs(sub_minigraph, mask,
                                  max_pair)  # 使用 get_all_pairs 函数为 sub_minigraph 生成一组对，最多 max_pair 对。
...
# %%
# %%
def train_batchlist(batches, gcnModel, rankNetModel, optimizer, criterion, device):
    # 这个函数的主要目的是训练模型并计算损失。
    ...
    for batch in batches:
        ...
        x = gcnModel(pyg1, del_index1)  # 通过 Model 计算 pyg1 和 pyg2 的输出。
...
def validate_batchlist(batches, gcnModel, rankNetModel, criterion, device):
    all_loss = []
    gcnModel.eval()
    rankNetModel.eval()  # 将 Model 和 rankNetModel 设置为评估模式。
...
def do_cross_fold_valid(device, K):
    ...
    for i in range(0, len(all_data_list)):
        ...
        mask = get_noise(train_x, train_y, psx, train_all_nodes, train_all_fdirs, train_all_labels,med)

        # mini_graphs = get_sub_minigraphs(trainset, all_mini_graphs)  # 获取训练集的生成图
